...
- Respect the worktree 7-day rule — single source of truth is the repo `CLAUDE.md` section "Worktree 7-day rule". A worktree touched within 7 days is protected and needs explicit `WORKTREE_APPROVED=1` user authorization. Recency comes from `scripts/lib/worktree_recency.sh`; never re-derive it from `stat <wt>/.git` or `stat <wt>` (both measure worktree creation, not use), and treat unmeasurable as protected.
