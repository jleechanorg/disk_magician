...
The node ceiling is an emergency infinite-run guard, not a normal coverage budget. Its default is 10,000,000 visited paths; the 480-second wall-clock cap should stop ordinary scans first. The human-readable report must show both nodes processed and this ceiling.
...
The top-down scanner prefers installed GNU `gdu` for fast allocated-size aggregation, then falls back to `dua` and bounded macOS `du`. The fast backends must not be wrapped in `taskpolicy -b`, which can starve them under host I/O pressure. Regular files use native allocated blocks (`st_blocks`) without a subprocess. Failed or partial commands never enter the accepted ledger, and symlink leaves never pull their targets into it.
