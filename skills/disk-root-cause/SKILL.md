...
The node ceiling is an emergency malformed/infinite-run guard, not a normal coverage budget. Its default is 100,000,000 inventory records (or visited paths in the legacy fallback); the 480-second wall-clock cap should stop ordinary scans first. The human-readable report must show the backend, records/paths processed, and this ceiling.
...
| **lane-frontier** | Run `disk_frontier_scan.py --granularity-gib 5 --output-default` (480s cap) | One-pass GNU du inventory when available; named frontier fallback otherwise |
