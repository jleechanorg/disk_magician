...
- The one-minute observer shows Colima flat near 33.20 GiB through 16:58, then growing to 43.44 GiB under 7-13 ephemeral self-hosted CI containers. The scheduled trim guard repeatedly skipped on a stale singleton lock and was therefore non-functional. However, the incident still had 46 GiB free, above the guard's hard-coded 40 GiB trigger, so a healthy guard would also have permitted growth to that point. The lock fix restores the safety net; it does not make the current threshold proactive.
...
- **Correction 2026-07-17 (later pass):** the previous version of this bullet cited `jleechan-6547` as tracking the prevention-policy gap below; that bead ID does not exist in `br` (confirmed via `br show` returning `ISSUE_NOT_FOUND` and keyword search returning nothing) — it was never actually created despite being asserted as real. The gap itself is genuine and was independently reproduced: `jleechan-uio3` now tracks it for real. Fresh evidence at the time of this correction: Colima had grown to 39.26 GiB (up from the 26.3 GiB cited a few minutes earlier) while host free space held at 57 GiB — comfortably above the guard's 40 GiB trigger the whole time, so the guard correctly skipped on every check per current policy even as Colima nearly quadrupled from its post-recovery low of 9.60 GiB. `jleechan-uio3` tracks deriving a proactive Colima-size-based trigger independent of host-wide free space.
